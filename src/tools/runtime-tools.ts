 ...
 * live game control. They use EditorBridge (or any desktop automation)
...
export function registerRuntimeTools({ server, reader, writer }: RuntimeToolDeps) {
  ...
  server.tool(
    'inject_runtime_bridge',
    'Inject the C3 runtime bridge script into the project. This enables external tools (EditorBridge, Playwright) to control the running game via globalThis.__c3bridge. The bridge script auto-runs via runOnStartup() and processes commands each tick.',
    {},
    async () => {
      try {
    ...
    async () => {
      const commands = {
        ...
        evaluateExpression: {
          ...
          args: {
            objectName: 'string (object type name, e.g. "MyStudioPlatformConnect")',
  ...
  server.tool(
    'generate_bridge_eval_script',
    'Generate a shell command (using curl or python) that can be run on a EditorBridge worker to execute a bridge command in the running C3 game. The command interacts with the game via Firefox remote debugging.',
    {
...
# This script should be executed via EditorBridge's exec_command or exec_python
      ...
      try {
        ...
        return toolResult({
          ...
          nextSteps: [
            ...
            'Or use EditorBridge to automate the entire flow',
          ],
        });
      } catch (error) {
