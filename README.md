...
| `generate_bridge_eval_script` | Generate a EditorBridge-compatible script to execute a bridge command |
...
The runtime bridge enables external tools (EditorBridge, Playwright) to control a running C3 game. Once injected and the game is previewed, you can:

```javascript
// From the browser console or via EditorBridge exec
...
See the [MyStudio Construct MCP](https://github.com/mystudio/mystudio-construct-mcp) for a domain-specific layer built on top of these runtime tools for slot machine testing.
...
- [x] EditorBridge eval script generation
...
- **Runtime Bridge Requires Desktop Automation**: The runtime tools inject a bridge script but need an external tool (EditorBridge, Playwright) to drive the browser and interact with the running game
